        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {doc.content && (
            <div style={{
              ...
              <h3 style={{ margin: "0 0 14px", fontSize: 13, fontWeight: 700, color: "var(--text-primary)" }}>
                Isi Dokumen
              </h3>
              ...
              {doc.type === "INVOICE" && doc.content.items && (
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                    <thead>
                      <tr>
                        {["No","Deskripsi","Qty","Harga","Total"].map((h) => (
                          <th key={h} style={{
                    ...
                    <tbody>
                      {doc.content.items.map((item: any, i: number) => (
                        <tr key={i} style={{ borderTop: "1px solid var(--table-border)" }}>
                          ...
                          <td style={{ padding: "8px 10px", color: "var(--text-secondary)" }}>{item.qty} {item.unit}</td>
                          <td style={{ padding: "8px 10px", color: "var(--text-secondary)" }}>{formatRp(item.price)}</td>
                          <td style={{ padding: "8px 10px", color: "var(--primary)", fontWeight: 700 }}>{formatRp(item.total)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                    ...
                    {[
                      ...
                      <div key={r.l} style={{ display: "flex", gap: 24, fontSize: 12 }}>
                        ...
                        <span style={{
                          ...
                          color: r.bold ? "var(--primary)" : "var(--text-primary)",
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
                    {[doc.content.party1, doc.content.party2].map((party: any, i: number) => (
                      <div key={i} style={{
                        ...
                        <div style={{ fontSize: 10, fontWeight: 700, color: "var(--primary)", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                          Pihak {i + 1}
                        </div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)", marginBottom: 2 }}>{party?.name ?? "-"}</div>
                        <div style={{ fontSize: 11, color: "var(--text-muted)" }}>{party?.rep ?? "-"}</div>
                      </div>
                    ))}
                  </div>
