...
function SalesDetailModal({
  ...
  userId: string; userName: string; userRole: string; onClose: () => void
}) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/sales-detail?${params}`, {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:"16px" }}>
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        {/* Header */}
        <div style={{ padding:"14px 18px", borderBottom:"1px solid var(--border)", display:"flex", justifyContent:"space-between", alignItems:"center", gap:10, flexShrink:0, flexWrap:"wrap" }}>
          <div style={{ minWidth:0 }}>
            <h3 style={{ margin:"0 0 2px", fontSize:14, fontWeight:700, color:"var(--text-primary)", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
              Performa — {userName}
            </h3>
            <p style={{ margin:0, fontSize:11, color:"var(--text-muted)" }}>{ROLE_LABEL[userRole] ?? userRole}</p>
          </div>
          <div style={{ display:"flex", gap:6, alignItems:"center", flexShrink:0 }}>
            <select value={year} onChange={(e) => setYear(e.target.value)} style={{ padding:"4px 8px", background:"var(--bg-card2)", color:"var(--text-secondary)", border:"1px solid var(--border)", borderRadius:6, fontSize:11, cursor:"pointer" }}>
              {YEARS.map((y) => <option key={y} value={y}>{y}</option>)}
            </select>
            <select value={month} onChange={(e) => setMonth(e.target.value)} style={{ padding:"4px 8px", background:"var(--bg-card2)", color:"var(--text-secondary)", border:"1px solid var(--border)", borderRadius:6, fontSize:11, cursor:"pointer" }}>
              <option value="all">Semua Bulan</option>
              {MONTHS.map((m, i) => <option key={i+1} value={String(i+1)}>{m}</option>)}
            </select>
            <button onClick={onClose} style={{ width:28, height:28, borderRadius:7, background:"var(--bg-card2)", border:"1px solid var(--border)", cursor:"pointer", color:"var(--text-muted)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0 }}>
              &times;
            </button>
          </div>
        </div>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"14px 18px" }}>
          ...
          {error && (
            <div style={{ padding:"10px 13px", background:"var(--danger-pale)", border:"1px solid rgba(239,68,68,0.2)", borderRadius:8, fontSize:13, color:"var(--danger)", marginBottom:12 }}>
              {error} — <button onClick={load} style={{ background:"none", border:"none", color:"var(--primary)", cursor:"pointer", fontSize:13 }}>Coba lagi</button>
            </div>
          )}
          ...
          {loading && !data && (
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10 }}>
                {[0,1,2,3].map((i) => (
                  <div key={i} style={{ background:"var(--bg-card2)", borderRadius:10, padding:"12px", border:"1px solid var(--border)" }}>
                    <Sk w="55%" h={10} /> <div style={{ marginTop:8 }}><Sk w="75%" h={20} /></div>
                  </div>
                ))}
              </div>
                ...
                {[0,1].map((i) => (
                  <div key={i} style={{ background:"var(--bg-card2)", borderRadius:10, padding:14, border:"1px solid var(--border)", height:130 }}>
                    ...
                    <div style={{ marginTop:10, display:"flex", flexDirection:"column", gap:8 }}>
                      <Sk h={12} /><Sk h={12} /><Sk h={12} />
                    </div>
                  </div>
                ))}
              </div>
              {[0,1,2,3].map((i) => (
                <div key={i} style={{ display:"flex", gap:10, alignItems:"center", padding:"10px 12px", background:"var(--bg-card2)", borderRadius:8, border:"1px solid var(--border)" }}>
                  <Sk w="8px" h={8} /> <Sk w="50%" h={12} /> <Sk w="60px" h={18} />
                </div>
              ))}
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10, marginBottom:14 }}>
                {[
                  ...
                  <div key={s.l} style={{ background:"var(--bg-card2)", borderRadius:10, padding:"11px 12px", border:"1px solid var(--border)", borderTop:`2px solid ${s.c}` }}>
                    <div style={{ fontSize:9, color:"var(--text-muted)", marginBottom:4, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.05em" }}>{s.l}</div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:14 }}>
                    ...
                    return (
                      <div key={s.l} style={{ marginBottom:8 }}>
                        ...
                        <div style={{ height:4, background:"var(--bg-card)", borderRadius:999, overflow:"hidden" }}>
                          <div style={{ height:"100%", width:`${pct}%`, background:s.c, borderRadius:999, transition:"width .5s ease" }} />
                        </div>
                      </div>
                    )
                  })}
                ...
                <div style={{ background:"var(--bg-card2)", borderRadius:10, padding:"13px 14px", border:"1px solid var(--border)" }}>
                  ...
                  {Object.keys(statusDist).length === 0 ? (
                    <p style={{ fontSize:12, color:"var(--text-muted)", textAlign:"center", padding:"16px 0" }}>Tidak ada lead</p>
                ...
                {filtered.length === 0 ? (
                  ...
                  return (
                    <div key={lead.id} style={{
                      ...
                      borderRadius:8, border:`1px solid ${isDeal?"rgba(16,185,129,0.14)":"var(--border)"}`,
                    }}>
                      ...
                      <span style={{ fontSize:10, fontWeight:700, padding:"2px 7px", borderRadius:999, background:sc+"18", color:sc, flexShrink:0, whiteSpace:"nowrap" }}>{sl}</span>
                      {lead.value !== null && (
